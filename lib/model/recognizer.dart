...
class Recognizer {
  int mnistImgSize = Constants.mnistImageSize;
  double canvasSize = Constants.canvasSize;
  double imgSize = Constants.imageSize;
  ...
  Future<Uint8List> previewImage(List<Offset> points) async {
    ...
    final image = await picture.toImage(mnistImgSize, mnistImgSize);
  ...
  Future recognize(List<Offset> points) async {
    final picture = _pointsToPicture(points);
    Uint8List bytes = await _imageToByteListUint8(picture, mnistImgSize);
  ...
  Future<Uint8List> _imageToByteListUint8(Picture pic, int size) async {
    ...
    final resultsBytes = Float32List(size * size);
    final buffer = Float32List.view(resultsBytes.buffer);
    ...
    return resultsBytes.buffer.asUint8List();
  }

  Picture _pointsToPicture(List<Offset> points) {
    final recorder = PictureRecorder();
    final canvas = Canvas(recorder, _canvasCullRect)
      ..scale(mnistImgSize / canvasSize);

    canvas.drawRect(Rect.fromLTWH(0, 0, imgSize, imgSize), _bgPaint);
